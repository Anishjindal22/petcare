    ...
    try {
        ...
        if(!email){
            return res.status(402).json({
                success:false,
                messagfe:"All fields are required"
            })
        }
        ...
        if(!existingUser){
            return res.status(500).json({
                success:false,
                message:"User is not registerd with us"
            })
        }
        ...
        const updatedUser = await User.findByIdAndUpdate({email},
            ...
            await mailSender(email,"password rsett link",`${url}`);
        ...
        return res.status(500).json({
            success:false,
            message:"Something went wrong while sensing mail"
        })
    }
}
